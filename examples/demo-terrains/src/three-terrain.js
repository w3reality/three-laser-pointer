...
// @@ A hackish version of 'THREE.Terrain.js' with mods:
//   - Wrapped as 'esm',
//   - Added Compatibility with three.js >= r125,

const THREE = { Terrain: {} }; // First, it's hollow!!

const getModule = EXTERN_THREE => Object.assign(THREE, EXTERN_THREE).Terrain;
...
const __ge125UpdateVertices = (geom, fn) => {
    const { array, count } = geom.attributes.position;

    // temporay copy of vertex data
    const verts = new Array(count).fill()
        .map((_, idx) => new THREE.Vector3(
            array[3 * idx], array[3 * idx + 1], array[3 * idx + 2]));

    fn(verts);

    verts.forEach((vert, idx) => {
        array[3 * idx] = vert.x;
        array[3 * idx + 1] = vert.y;
        array[3 * idx + 2] = vert.z;
    });
};
//----
...
THREE.Terrain = function(options) {
    ...
    console.log('@@ mesh.geometry:', mesh.geometry); // For three-r125 and later, `BufferGeometry` NOT `Geometry`
    ...
    if (options.heightmap instanceof HTMLCanvasElement || options.heightmap instanceof Image) {
        if (__ge125()) { // @@
            __ge125UpdateVertices(mesh.geometry,
                verts => THREE.Terrain.fromHeightmap(verts, options));
        } else {
        ...
        if (__ge125()) { // @@
            __ge125UpdateVertices(mesh.geometry,
                verts => options.heightmap(verts, options));
        } else {
    ...
    if (options.useBufferGeometry) {
        if (!__ge125()) { // @@ for three r125 and later, NOP since already `bufferGeometry`
...
THREE.Terrain.Normalize = function(mesh, options) {
    if (__ge125()) { // @@
        __ge125UpdateVertices(mesh.geometry, verts => {
            if (options.turbulent) {
                THREE.Terrain.Turbulence(verts, options);
            }
            if (options.steps > 1) {
                THREE.Terrain.Step(verts, options.steps);
                THREE.Terrain.Smooth(verts, options);
            }
            // Keep the terrain within the allotted height range if necessary, and do easing.
            THREE.Terrain.Clamp(verts, options);

            // Call the "after" callback
            if (typeof options.after === 'function') {
                options.after(verts, options);
            }
        });
    } else {
...
THREE.Terrain.ScatterMeshes = function(geometry, options) {
    ...
    //     return;
    // }
    //==== @@ workaround applied in the following
    ...
    for (var i = 0, w = options.w*2; i < w; i++) {
        for (var j = 0, h = options.h; j < h; j++) {
            ...
            if (__ge125()) { // @@ support only minimal location adjustment
                if (Math.random() < 0.01) {
                    const mesh = options.mesh.clone();
                    const rnd = Math.random();

                    // Randomly pick up a position on the terrain
                    const { array, count } = geometry.attributes.position;
                    const idx = Math.floor(count * rnd * 0.9);
                    mesh.position.x = array[3 * idx];
                    mesh.position.y = array[3 * idx + 1];
                    mesh.position.z = array[3 * idx + 2];

                    mesh.rotation.x += 90 / 180 * Math.PI;
                    mesh.rotateY(rnd * 2 * Math.PI);
    ...
    if (!__ge125() && options.mesh.geometry instanceof THREE.Geometry) {
...
export default getModule; // @@ hackish
